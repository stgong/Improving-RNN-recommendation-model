...
def command_parser():
    ...
    parser.add_argument('-f', dest='filename', help='Input file',
                        default='/Users/xun/Documents/Thesis/Improving-RNN-recommendation-model/ks-3m/ks-3m.train.rating.csv', type=str)
...
def load_data(filename, columns, separator):
    ...
    data = pd.read_csv(filename, sep=separator, names=list(columns), index_col=False, header=None, usecols=range(len(columns)))
...
def split_data(data, nb_val_users, dirname, save_time = False):
    ...
    val_set, train_set = extract_n_users(data, nb_val_users)

    print('Save training and validation in the triplets format...')
...
def make_sequence_format(train_set, val_set, dirname, columns):
...
def save_data_stats(data, train_set, val_set, dirname):
    print('Save stats...')
...
def main():
    ...
    train_set, val_set = split_data(data, args.val_size, args.dirname, args.min_time_diff)
    make_sequence_format(train_set, val_set, args.dirname, args.columns)
    save_data_stats(data, train_set, val_set, args.dirname)
    # 	make_readme(args.dirname, val_set, args.min_time_diff)
